...
class KafkaLocalConsumer implements ObservationCollector<KafkaObservation> {
    ...
    private static final int MAX_MESSAGES_TO_FETCH_FROM_KAFKA = 10000;
    /**
     ...
     */
    KafkaLocalConsumer() {
        kafkaconsumer = new SimpleConsumer("localhost", KafkaDataLoader.KAFKA_BROKER_PORT, 10000, 1024000, "test");
    }
    ...
    private List<String> getMessages(String topic) {
        ...
        FetchRequest request = new FetchRequestBuilder().clientId("clientName").addFetch(topic, 0, offset, MAX_MESSAGES_TO_FETCH_FROM_KAFKA).build();
